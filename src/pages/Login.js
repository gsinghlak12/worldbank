...
function Login(props) {
	...
	const postLogin = async (e) => {
		...
		const response = await fetch(`${config.wfwApi}/users/verify`, requestOptions);
	...
	async function postSession() {
		setSessionCount(1);
		try {
			const response = await fetch(`${config.wfwApi}/sessions`, {
				method: 'POST',
				credentials: 'include',
				headers: {
					'Content-Type': 'application/json',
	...
	async function checkSessionExists() {
		setSessionUpdate(1);
		try {
			const response = await fetch(`${config.wfwApi}/sessions/cookie`, {
				method: 'GET',
				credentials: 'include',
				headers: {
					'Content-Type': 'application/json',
