...
function Register(props) {
	...
	const validatePassword = () => {
		...
		if (password !== confirm) {
			return 'match';
		} else if (!password || !confirm || password.length < 9 || !specChar.test(password)) {
			return 'weak';
		}
	};
	...
	const handleNewUser = () => {
		...
		if (usernameResult === 'invalid') {
			setMessage({
				error: 'Your username must be at least 6 characters long and contain a number.',
			});
			...
			setMessage({
				error: 'Password is weak. Must be 8 characters long and contain special characters.',
			});
		} else {
			postNewUser();
		}
